...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^legion_") ; then
        LEGION_BUILD=$(echo -n $1 | sed -e 's/^legion_//g')
    else
        LEGION_BUILD=
    fi
    export LEGION_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the LEGIONOS GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/legion/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/legion/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
